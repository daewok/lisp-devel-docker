...
# If the UID is different than 0, we must change the owner of everything in
# /home/lisp to UID. However, we'd like to not touch anything that's been
# mounted into the container. Presumably, the user is requesting a specific UID
# because of some constraints with sharing volumes, so we assume that mounted
# volumes have the owners that the user really desires.
if [ "${_UID}" != 0 ]; then
